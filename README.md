...
Next.js 16 app (App Router, `src/`) on Vercel. Postgres (Neon) via Drizzle holds users/sessions, a cache of HubSpot Project data (`books` + `book_cache`), admin-editable configuration (pipeline stages, action rules, friendly labels, Drive file visibility, handbook versions), chat history, and an audit log. Two Vercel Cron jobs hit `/api/cron/sync` every 10 minutes (incremental) and nightly (full reconcile, paginated and resumable to handle 20k+ authors within the function time limit). The app itself never calls HubSpot at request time — pages read only from Postgres, scoped to the signed-in user by helpers in `src/lib/data/`. Drive files stream through the portal after the same ownership check, via a service account. The assistant (Vercel AI SDK) loads the whole Author Handbook into the prompt with provider-side caching, on whichever of Anthropic / OpenAI / Google an admin has configured and selected in `/admin/assistant`.
